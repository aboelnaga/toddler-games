...
STYLE_FRAGMENT = (
    ...
    "pure flat magenta chroma-key background. The character has an "
    "oversized round head, big round eyes with a small white catchlight "
    "in each eye, a tiny rounded snout/nose, and a chunky compact body "
    "with short limbs — no realistic anatomy. The character has NO "
    "outlines and NO black ink lines anywhere on its body; form is built "
    "from clean flat shapes with gentle inner shading. Reference "
    "exemplars: Sago Mini, Hey Duggee, Toca Boca. Mood: warm, calm, "
    "friendly, safe for a 2-year-old. Square aspect ratio, 2048x2048, "
    "centered subject filling about 70% of the frame, generous negative "
    "space around the character. No text, no decorative lettering, no "
    "logos.\n\n"
    "Background: a single pure flat magenta #ff00ff tone (chroma-key "
    "colour) — NO watercolor, NO gradient, NO atmospheric wash, NO sky "
    "or meadow. The character must appear isolated on flat magenta so "
    "the background can be cleanly removed later. NO baked-in drop "
    "shadow beneath the character (shadows are added programmatically "
    "in the app).\n\n"
    ...
    "decorative text, watercolor backdrops, gradient backgrounds, "
    "atmospheric washes, drop shadows under the character, magenta "
    "anywhere ON the character (it is reserved for the background "
    "only).\n\n"
